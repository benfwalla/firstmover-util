...
export const iconMap: { [key: string]: React.ReactNode } = {
  ...
  property: <BuildingIcon className="h-5 w-5" />,
...
export type LocationFeature = {
  ...
  properties: {
    ...
    // Additional fields for open houses
    openHouseDate?: string;
    openHouseTime?: string;
    price?: string | number;  // Allow both string and number types for price
    bedrooms?: number;
    bathrooms?: number;
    sqft?: number;
    propertyType?: string;
...
// For open house listings specifically
export type OpenHouseListing = LocationFeature & {
  properties: LocationFeature['properties'] & {
    ...
    availableAt: string;
    photoUrl: string;
    price: number;  // This will override the parent's string|number type
    priceDisplay: string;
    propertyType: string;
    // Optional fields that may not be present in all listings
    listingAgent?: string;
    listingAgentPhone?: string;
    description?: string;
  };
